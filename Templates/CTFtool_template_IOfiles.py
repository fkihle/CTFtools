...
################################################################
def ctf_tool(input_file_path, output_file_path):
    ...
    try:
        with open(input_file_path, 'r', encoding='latin-1') as infile, \
             open(output_file_path, 'w', encoding='latin-1') as outfile:
            for line in infile:
...
def main(argv):
    ...
    if not opts:
        logger.error('Expected: python <CTFtool name>.py -i <inputfile> -o <outputfile>\n')
        sys.exit()
    ...
    for opt, arg in opts:
        ...
        if opt == '-h':
            print('python <CTFtool name>.py -i <inputfile> -o <outputfile>')
            sys.exit()
        elif opt in ("-i", "--ifile"): # If the user specifies the input file
            inputfile = arg
        elif opt in ("-o", "--ofile"): # If the user specifies the output file
            outputfile = arg

    # Print the input and output file paths (nice for debugging)        
    logger.info('Input file is ', inputfile)
    logger.info('Output file is ', outputfile)
    
    # Call the CTF tool function
    ctf_tool(inputfile, outputfile)
